...
import { API_CONFIGS, getApiKey, mapCreativityToTemperature, callAdforgeAI } from "@/lib/ai-providers";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (provider === "adforge") {
      ...
      const aiResult = await callAdforgeAI(prompt, temperature);
      raw = aiResult.raw;
      tokensUsed = aiResult.tokensUsed;
    } else {
      // External provider — needs API key
      if (!apiKey) {
    ...
    // posterPrompt is optional but very useful — ensure it exists
    if (!result.posterPrompt || typeof result.posterPrompt !== "string") {
      // Generate a fallback poster prompt from the campaign content
      result.posterPrompt = `Professional advertisement poster for "${productName}". Headline: "${result.headline}". ${result.tagline ? `Tagline: "${result.tagline}".` : ""} Style: modern, clean, eye-catching. Professional graphic design, commercial grade.`;
    }
