...
const ALL_PROVIDERS = [
  { id: "adforge", label: "AdForge AI", model: "Built-in" },
...
function GenerateTab() {
  ...
  const handleGeneratePosters = useCallback(async () => {
    if (!store.result) {
      toast.error("Generate a campaign first before creating posters");
      return;
    }

    store.setPosterLoading(true);
    store.setPosterError("");
    ...
    try {
      const res = await fetch("/api/generate-poster", {
        ...
        body: JSON.stringify({
          ...
          tone: store.resultTone,
          platforms: store.resultPlatforms,
          posterStyle: (store.result as any).posterPrompt || "",
        }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Poster generation failed");

      store.setPosters(data.posters);
      toast.success(`${data.posters.length} poster(s) generated!`);
      if (data.errors?.length) {
        toast.warning(`Some posters failed: ${data.errors.join(", ")}`);
      }
    } catch (err: any) {
      store.setPosterError(err.message || "Failed to generate posters");
      toast.error(err.message || "Poster generation failed");
    }
  }, [store.result, store.resultProductName, store.resultTone, store.resultPlatforms, store.productDesc]);

  // ── Export Professional PDF with Posters ────────────────────────
  const handleExportPDF = useCallback(async () => {
    ...
    try {
      ...
      toast.success("Professional PDF ready for printing/saving!");
    } catch (err: any) {
      toast.error(err.message || "PDF export failed");
    }
  }, [store.result, store.resultProductName, store.resultTone, store.resultPlatforms, store.productDesc, store.posters]);
  ...
  const handleExport = useCallback(
    ...
    [store.result, store.resultProductName, handleExportPDF]
  );
      ...
      <div className="lg:sticky lg:top-[57px] lg:self-start lg:max-h-[calc(100vh-57px)] lg:overflow-y-auto custom-scrollbar pb-6 lg:pb-0">
        <Card className="border-0 rounded-lg overflow-hidden">
          ...
          <div className="p-5 space-y-5">
            ...
            <div>
              ...
              <div className="flex gap-2">
                {ALL_PROVIDERS.map((p) => {
                  ...
                  const isServerAvailable = serverInfo?.available ?? false;
                  const hasUserKey = store.apiKey.trim().length > 0;
                  // Provider is available if server has key OR user provided their own key
                  const isAvailable = isServerAvailable || hasUserKey;
            ...
            {/* 1b. API Key — user can provide their own key */}
            <div>
              <Label className="text-[11px] uppercase tracking-wider font-bold text-ink-soft mb-2 flex items-center gap-1.5">
                <Key className="w-3 h-3" />
                API Key
                {availableProviders.find(ap => ap.id === store.provider)?.available ? (
                  <span className="text-[9px] font-normal text-status-green">Server key active</span>
                ) : (
                  <span className="text-[9px] font-normal text-amber-600">Required — no server key</span>
                )}
              </Label>
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <button
                    type="button"
                    onClick={() => store.setApiKeyVisible(!store.apiKeyVisible)}
                    className="absolute right-2.5 top-1/2 -translate-y-1/2 text-ink-muted hover:text-ink-soft transition-colors"
                  >
                    {store.apiKeyVisible ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                  </button>
                </div>
              </div>
              <p className="text-[10px] text-ink-muted mt-1.5 leading-relaxed">
                {availableProviders.find(ap => ap.id === store.provider)?.available
                  ? "A server key is already configured. Your own key will override it if entered."
                  : "No server key configured for this provider. Enter your own key to use it."}
                {" "}Your key stays in your browser — never sent to our database.
              </p>
            </div>
          ...
          <div className="space-y-4">
            ...
            >
              <Card className="border-l-4 border-l-purple-500 hover:shadow-md transition-shadow">
                <div className="p-5">
                  ...
                  {store.posterError && (
                    <div className="p-3 bg-red-50 border border-red-200 rounded-md text-red-600 text-xs mb-3">
                      {store.posterError}
                    </div>
                  )}

                  {store.posters.length === 0 && !store.posterLoading && (
                    <div className="text-center py-6 border-2 border-dashed border-purple-200 rounded-lg bg-purple-50/50">
                      <ImageIcon className="w-8 h-8 text-purple-300 mx-auto mb-2" />
                      <p className="text-xs text-purple-500">
                        Click "Generate Posters" to create AI-powered visual ad creatives for each platform
                      </p>
                    </div>
                  )}
                  ...
                  {store.posters.length > 0 && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {store.posters.map((poster, i) => (
                        <motion.div
                          key={poster.platform}
                          initial={{ opacity: 0, scale: 0.95 }}
                          animate={{ opacity: 1, scale: 1 }}
                          transition={{ delay: 0.1 * i }}
                          className="border border-border rounded-lg overflow-hidden bg-white hover:shadow-md transition-shadow group relative"
                        >
                          <div className="relative">
                            <img
                            ...
                            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                              <Button
                                ...
                                className="text-xs"
                                onClick={() => {
                                  // Download individual poster
                                  const link = document.createElement("a");
                                  link.href = `data:${poster.mimeType};base64,${poster.imageBase64}`;
                                  link.download = `${store.resultProductName.replace(/\s+/g, "-").toLowerCase()}-${poster.platform}-poster.png`;
                                  link.click();
                                  toast.success(`${poster.label} poster downloaded!`);
                                }}
                              >
                          ...
                          <div className="px-3 py-2 bg-cream/50 border-t border-border flex items-center justify-between">
                            <span className="text-[10px] font-bold text-purple-700 uppercase tracking-wider">{poster.label}</span>
                            <span className="text-[10px] text-ink-muted">{poster.size}</span>
                          </div>
                        </motion.div>
                      ))}
                    </div>
            ...
            <div className="flex justify-center gap-3 pt-2">
              <Button
                ...
                className="bg-terracotta hover:bg-terracotta-dark text-white px-6"
                size="lg"
              >
                <FileText className="w-4 h-4 mr-2" />
                Export PDF with Posters
              </Button>
              <Button
                onClick={handleSave}
